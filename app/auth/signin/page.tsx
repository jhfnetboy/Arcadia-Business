...
function SignInContent() {
  ...
  const handleSignIn = async (provider: string) => {
    try {
      ...
      // 记录登录尝试
      console.log(`Attempting to sign in with ${provider}`, { callbackUrl })
      
      // 使用NextAuth.js的signIn函数
    ...
    <div className="w-full max-w-md rounded-lg border border-gray-200 bg-white p-6 shadow-md">
      ...
      <div className="space-y-3">
        <button
          ...
          className="flex w-full items-center justify-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-700 shadow-sm hover:bg-gray-50 disabled:opacity-70"
        >
          {isLoading['google'] ? (
            <span>登录中...</span>
          ) : (
...
function LoadingState() {
  return (
    <div className="w-full max-w-md rounded-lg border border-gray-200 bg-white p-6 shadow-md">
      <h1 className="mb-4 text-center text-2xl font-bold">加载中...</h1>
